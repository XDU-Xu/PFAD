...
class Resizer(nn.Module):
    ...
    def forward(self, in_tensor):
        ...
        for dim, fov, w in zip(self.sorted_dims, self.field_of_view, self.weights):
            ...
            # for each pixel in the output-image it matches the positions the influence it from the input image (
            # along 1 dim only, this is why it only adds 1 dim to 5the shape). We then multiply, for each pixel,
            # its set of positions with the matching set of weights. we do this by this big tensor element-wise
            # multiplication (MATLAB bsxfun style: matching dims are multiplied element-wise while singletons mean
            # that the matching dim is all multiplied by the same number
